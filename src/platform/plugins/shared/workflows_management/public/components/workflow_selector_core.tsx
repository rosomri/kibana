 ...
 * that can be used in different contexts (modal, form, etc.)
 */
export const WorkflowSelectorCore: React.FC<WorkflowSelectorCoreProps> = ({
  ...
  const renderWorkflowOption = useCallback((option: any, searchValue: string) => {
    return (
      <>
  ...
  const handleWorkflowChange = useCallback(
    (newOptions: any[], event: any, changedOption: any) => {
      setIsPopoverOpen(false);

      if (changedOption.checked === 'on') {
        selectWorkflow(changedOption.id);
